...
Deploymentの説明で安全にPodを更新する方法の説明を行ったが、Deploymentでカバーできるのは、あくまでPodを更新するときだけである。本番の運用環境ではNodeをメンテナンスするためにNodeからPodを退避させるなど、Podが増えたり減ったりするケースがよくある。こういったケースでもPodを安全に退避させるための機能の一つがPod Disruption Budegt(PDB)である。サービスのダウンが発生しても問題ないアプリ以外は必須の設定である。

- minAvailable:「最低幾つのPodga利用可能な状態であるか」を指定する方法である。例えばminAvailableを5に指定した場合、5つのPodが利用可能である必要がある。
- maxUnavailable:「最大で幾つのPodが利用不可能な状態であるか」を指定する方法である。例えばmaxUnavailableを5に指定した場合、5つのPod概要不可能な状態でも問題ない。

これらのminAvailableやmaxUnavailableという指定された値を超えないように退避させるPodの数を制御する。
